...
keyword ~this~ becomes important. The ~this~ keyword is interpreted upon
...
#+begin_src buzz
  ...
  t.f()
#+end_src
In other words, the keyword ~this~ points to the context in which the function is
called. When the function is called standalone, there is no context and ~this~ is
~nil~. When a function is called from a table, the ~this~ keyword points to the
table.

Using the ~this~ keyword, you can write methods that access class attributes:
#+begin_src buzz
  # Class definition
  myclass = {
    .myattribute = 1
